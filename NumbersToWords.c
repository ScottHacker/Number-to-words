/*
source:
https://raw.githubusercontent.com/ScottHacker/Number-to-words/master/NumbersToWords.c
*/

/*
Compiled & tested with:
gcc -x c -std=c89 -pedantic-errors -Wall -Werror -Wextra -Wcomment -Wparentheses
-Wformat-zero-length NumbersToWords.c -o binary/NumbersToWords.c
*/
...
int Speak(int t)
{
	...
	for (g = length / 3.0 - 0.1; g >= 0; g--) {
		...
		/* Go through each number in the group and translate it to a
		 * word properly. */
		while (i != 0) {
			/* If it's in the hundreds, then we need to add
			 * "Hundred" and possibly "and" afterwards if there's
			 * more. */
			if (i == 100) {
				...
				/* If it's anything else, then let's just say
				 * it.  We check if it's zero, which indicates
				 * that we're done with this loop.
				 */
		...
		if (group != 0) {
			...
			if (t >= 100)
				printf(", ");
			/* If all that's left is less than 100 but greater than
			 * 0, then we need an 'and'. For example: 5001 should
			 * equal "Five-Thousand and One".
			 */
			else if (t > 0) {
				printf(" and ");
			}
